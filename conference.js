...
export default {
    ...
    async startConference(conP, tracksP, connectionAndTracksPromiseFunction) {
        ...
        } = APP.store.getState()['features/base/track-operations'];
        const promiseToDispatch = Promise.allSettled([ audioTrackOperationsPromise, videoTrackOperationsPromise ])
        . then(() =>
        ...
        APP.store.dispatch(setTrackOperationsPromise(promiseToDispatch, promiseToDispatch));

        await promiseToDispatch;
    ...
    muteAudio(mute, showUI = true) {
        ...
        if (!localAudio && !mute) {
            ...
            const { audioTrackOperationsPromise } = APP.store.getState()['features/base/track-operations'];
            const promise = audioTrackOperationsPromise.then(() =>
            ...
            APP.store.dispatch(setTrackOperationsPromise(promise));
    ...
    muteVideo(mute, showUI = true) {
        ...
        if (!localVideo && !mute) {
            ...
            const { videoTrackOperationsPromise } = APP.store.getState()['features/base/track-operations'];
            const promiseToDispatch = videoTrackOperationsPromise
            ...
            APP.store.dispatch(setTrackOperationsPromise(undefined, promiseToDispatch));
    ...
    _createRoom(localTracks) {
        ...
        const promiseToDispatch = Promise.allSettled([ audioTrackOperationsPromise, videoTrackOperationsPromise ])
            .then(() => this._setLocalAudioVideoStreams(tracks));
    ...
    _setupListeners() {
        ...
        APP.UI.addListener(
            ...
            cameraDeviceId => {
                ...
                const promise = videoTrackOperationsPromise.then(() => {
                    const localVideo = getLocalJitsiVideoTrack(APP.store.getState());
                ...
                APP.store.dispatch(setTrackOperationsPromise(undefined, promise));
            }
        );
